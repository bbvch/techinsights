...
That sentence was true when I wrote it. It is not true anymore.

Clone mode now *additionally* mounts the hosts repository read-only into the sandbox
-> do not use --clone if you have .env files that the agent should not see

See [the appendix](/sandbox-appendix#b-docker-sbx) for the commands on how to get the old guarantee back and more details on the issue.
...
**Content:** Jeremy. Proofread by [Raphael Schweizer](https://github.com/CaringDev) and Claude (LLM by Anthropic)
