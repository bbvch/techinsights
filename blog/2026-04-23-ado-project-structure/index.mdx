...
**Teams** are configurable views into the project's shared work item pool. When you create a team, Azure DevOps automatically provisions a backlog, Kanban board, sprint view, and dashboard for it. A team does not _own_ work items — it _filters_ them. This distinction shapes every configuration decision that follows.
...
- **Area Paths represent products.** A work item's area path tells you _which product_ it belongs to, not _which team_ is working on it.
- **Subiterations represent team assignment within a sprint.** Each sprint node in the iteration hierarchy contains one child node per team. A work item's iteration path tells you both which sprint it belongs to _and_ which team is responsible for it in that sprint.
...
**Creating subiterations but not updating team iteration settings.** When you add a new subiteration, each team must go to _Project Settings → [Team] → Iterations_ and mark it as active. This is a common operational mistake with the subiteration pattern and easy to overlook during sprint kickoff.
